...
void modArray(int nums[], int size)
{
	...
	while (go)
	{
		try
		{
			...
			if (modIndex < 0 || modIndex > 99)
			{
				throw out_of_range("You did not choose a number between 0 and 99. Try again.");
			}

			if (cin)
			{
				cout << "(You entered a decimal. Used number " << modIndex << ")" << endl;
			...
			if (cin)
			{
				cout << "(You entered a decimal. Used number " << newInt << ")" << endl;
				cin.ignore(numeric_limits<int>::max(), '\n');
			}
...
int addEnd(int nums[], int size, int currSize)
{
	...
	while (true)
	{
		try
		{
			...
			if (cin)
			{
				cout << "(You entered a decimal. Used number " << userNum << ")" << endl;
				cin.ignore(numeric_limits<int>::max(), '\n');
			}
